...
def solver(
    pages: list[tuple[list[str], dict[str, list[str]]]]
) -> Iterator[str | int]:
    """
    ...
    yield sum(
        count_unique_names(name, rules)
        for name in clean_names(names, rules)
        )
...
def count_unique_names(
    ...
    if len(prefix) == 11:
        return 1
    return sum(
        count_unique_names(prefix + c, rules)
        for c in rules[prefix[-1]]
        ) + (
        len(prefix) > 6
    )
